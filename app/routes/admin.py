...
async def admin_tenant_create_submit(
    ...
    nginx_snippet_text = (
        ...
        f"    rewrite ^/api/{safe_slug}/(.*) /$1 break;\n"
    ...
    deploy_script_text = (
        ...
        f"set -e\n"
        f"\n"
        f"mkdir -p /root/clients/{safe_slug}/config /root/clients/{safe_slug}/data\n"
        f"cd /root/clients/{safe_slug}\n"
        f"\n"
        f"# 1. Paste client.json into:    /root/clients/{safe_slug}/config/client.json\n"
        f"# 2. Paste platform.env into:   /root/clients/{safe_slug}/platform.env\n"
